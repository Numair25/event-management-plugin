...
7. **Advanced Settings & Tools**
   - **Configurable Unique Validation:** Easily enforce duplicate checks on any Gravity Form field (e.g., Email, Phone Number, Employee ID) directly from the Settings menu.
   - **Historical Data Sync:** If you map new fields to Gravity Forms *after* attendees have registered, run the built-in sync tool to retroactively fetch missing data (like Phone Numbers) into the Attendees table.
   - **CSV Export:** Select an event and export all its attendees to a CSV file.

8. **Dynamic UPI QR Payments & Manual Approvals**
