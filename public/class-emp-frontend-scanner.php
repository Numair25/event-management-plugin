		...
		document.addEventListener("DOMContentLoaded", function() {
			...
			function processToken(token) {
				...
				$.ajax({
					...
					$('#scan-result').addClass('error').html('<h3>Error</h3><p>' + (jqXHR.responseJSON ? jqXHR.responseJSON.message : 'Connection Error') + '</p>');
					resumeScanner();
				});
			}
			...
			function showResult(response) {
				...
				if (response.success) {
					resultBox.css('background', '#d4edda');
				} else {
					resultBox.css('background', '#f8d7da');
				}
